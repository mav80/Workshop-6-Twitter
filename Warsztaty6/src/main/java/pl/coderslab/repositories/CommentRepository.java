...
public interface CommentRepository extends JpaRepository<Comment, Long>{
	...
	@Query(value = "SELECT comment.id, comment.text, comment.created, comment.receiver, comment.user_id, comment.tweet_id, editable FROM Warsztaty6Twitter.Comment AS comment JOIN(SELECT Tweet.id, Tweet.created, receiver, text, user_id FROM Warsztaty6Twitter.Tweet JOIN Warsztaty6Twitter.User ON Tweet.user_id=User.id WHERE User.deleted = 0 ORDER BY Tweet.created DESC) AS tweet ON comment.tweet_id = tweet.id ORDER BY comment.created ASC", nativeQuery = true)
	List<Comment> findAllOrderByCreatedAscOnlyForTweetsFromNotDeletedUsers();
	
	
	@Query(value = "SELECT id, text, created, receiver, user_id, tweet_id FROM (SELECT comment.id, comment.text, comment.created, comment.receiver, comment.user_id, comment.tweet_id, editable FROM Warsztaty6Twitter.Comment AS comment JOIN (SELECT Tweet.id, Tweet.created, receiver, text, user_id FROM Warsztaty6Twitter.Tweet JOIN Warsztaty6Twitter.User ON Tweet.user_id=User.id WHERE User.deleted = 0 ORDER BY Tweet.created DESC) AS tweet ON comment.tweet_id = tweet.id ORDER BY comment.created ASC) AS commentsFromNotDeletedUsers JOIN (SELECT id AS userid, deleted FROM Warsztaty6Twitter.User) AS user ON commentsFromNotDeletedUsers.user_id = user.userid WHERE user.deleted = 0 ORDER BY commentsFromNotDeletedUsers.created ASC", nativeQuery = true)
	List<Comment> findAllFromNotDeletedUsersOrderByCreatedAscOnlyForTweetsFromNotDeletedUsers();
	
	
	@Query(value = "SELECT id, text, created, receiver, user_id, tweet_id, editable FROM (SELECT comment.id, comment.text, comment.created, comment.receiver, comment.user_id, comment.tweet_id, editable FROM Warsztaty6Twitter.Comment AS comment JOIN (SELECT Tweet.id, Tweet.created, receiver, text, user_id FROM Warsztaty6Twitter.Tweet JOIN Warsztaty6Twitter.User ON Tweet.user_id=User.id WHERE User.deleted = 0 ORDER BY Tweet.created DESC LIMIT ?1 OFFSET ?2) AS tweet ON comment.tweet_id = tweet.id ORDER BY comment.created ASC) AS commentsFromNotDeletedUsers JOIN (SELECT id AS userid, deleted FROM Warsztaty6Twitter.User) AS user ON commentsFromNotDeletedUsers.user_id = user.userid WHERE user.deleted = 0 ORDER BY commentsFromNotDeletedUsers.created ASC", nativeQuery = true)
	List<Comment> findOnlyForTweetsOnPageFromNotDeletedUsersOrderByCreatedAscOnlyForTweetsFromNotDeletedUsers(int limit, int offset);
	
	
	
	
	@Query(value = "SELECT id, text, created, receiver, user_id, tweet_id, editable FROM (SELECT comment.id, comment.text, comment.created, comment.receiver, comment.user_id, comment.tweet_id, editable FROM Warsztaty6Twitter.Comment AS comment JOIN (SELECT Tweet.id, Tweet.created, receiver, text, user_id FROM Warsztaty6Twitter.Tweet JOIN Warsztaty6Twitter.User ON Tweet.user_id=User.id WHERE User.deleted = 0 ORDER BY Tweet.created DESC) AS tweet ON comment.tweet_id = tweet.id ORDER BY comment.created ASC) AS commentsFromNotDeletedUsers JOIN (SELECT id AS userid, deleted FROM Warsztaty6Twitter.User) AS user ON commentsFromNotDeletedUsers.user_id = user.userid WHERE user.deleted = 0 AND tweet_id = ?1 ORDER BY commentsFromNotDeletedUsers.created ASC", nativeQuery = true)
	List<Comment> findAllForAGivenTweetFromNotDeletedUsersOrderByCreatedAscOnlyForTweetsFromNotDeletedUsers(long tweetId);
